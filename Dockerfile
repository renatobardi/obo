...
COPY obo/__init__.py ./obo/__init__.py
...
# Point the app at the pre-baked tiktoken encoding (see obo/config.py)
ENV TIKTOKEN_CACHE_DIR=/app/tiktoken-cache
...
# enabled via OBO_ENABLE_* before handing off to CMD (supervisord).
