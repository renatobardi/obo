...
#     --ns obo --db <your-db> /dev/stdout > /tmp/dev-dump.surql
#
...
KEY=$(grep '^OBO_ENCRYPTION_KEY' "$REPO/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || true)
DB=$(grep '^SURREAL_DATABASE' "$REPO/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || true)

compose() {
  APP_IMAGE="ghcr.io/renatobardi/obo:$TAG" DATA_DIR="$RC_DATA" \
  API_PORT=15055 FE_PORT=18502 PROXY_PORT=18080 \
  RC_API_URL="http://localhost:15055" \
  RC_ENCRYPTION_KEY="${KEY:-release-test-key}" RC_SURREAL_DB="${DB:-obo}" \
...
case "$1" in
  ...
  up)
    ...
    docker pull "ghcr.io/renatobardi/obo:$TAG" || \
      echo "WARNING: could not pull ghcr.io/renatobardi/obo:$TAG — using the local image if present."
    ...
    if [ -n "$DUMP" ]; then
      docker exec onrelrc-surrealdb-1 /surreal import --conn http://localhost:8000 \
        --user root --pass root --ns obo --db "${DB:-obo}" \
        /mydata/dump.surql
    fi
    compose up -d
    ...
    echo "RC stack up — image ghcr.io/renatobardi/obo:$TAG"
