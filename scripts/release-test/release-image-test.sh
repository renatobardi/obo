...
#   e.g. release-image-test.sh all ghcr.io/renatobardi/obo:1.12.0 ghcr.io/renatobardi/obo:1.11.0
#
# Scenarios:
...
probe_test() {
  ...
  # #1141: OBO_WORKER_MAX_TASKS must reach the worker. supervisord's
  ...
  local CID
  CID=$(docker run -d --rm \
    -e OBO_WORKER_MAX_TASKS=2 \
    -e SURREAL_URL=ws://127.0.0.1:9/rpc \
    -e OBO_ENCRYPTION_KEY=probe \
  ...
  if [ -z "$CID" ]; then
    ...
    check "OBO_WORKER_MAX_TASKS honored by the in-image worker" "2" "$CONC"
  ...
  APP=$(docker run -d --rm --network "$NET" --name onrelprobe-app \
    ...
    -e SURREAL_NAMESPACE=obo -e SURREAL_DATABASE=probe \
    -e OBO_ENCRYPTION_KEY=probe \
