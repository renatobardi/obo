...
class TestSpeakerProfileResolve:
    ...
    async def test_resolve_by_record_id_queries_by_id(self):
        with patch(
            "obo.podcasts.models.repo_query", new_callable=AsyncMock
        ) as mock_query:
...
class TestOrphanedProfileDoesNotPoisonConfig:
    ...
    async def test_orphan_dropped_and_ids_rewritten_to_names(self, tmp_path):
        ...
        with (
            ...
            patch(
                "obo.podcasts.models._resolve_model_config",
            ...
            patch(
                "obo.podcasts.audio_paths.PODCASTS_FOLDER",
                str(tmp_path),
            ),
            patch(
            ...
            patch(
                "obo.podcasts.models.PodcastEpisode.save",
                new=AsyncMock(),
            ),
        ):
