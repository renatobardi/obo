...
from obo.exceptions import DatabaseOperationError, InvalidInputError
...
class TestAddInsightRaisesOnSubmissionFailure:
    ...
    async def test_returns_command_id_on_success(self):
        source = make_source()
        with patch(
            "obo.domain.notebook.submit_command",
...
class TestTransformationGraphPropagatesFailure:
    """obo/graphs/transformation.py: run_transformation()."""

    @pytest.mark.asyncio
    async def test_add_insight_failure_propagates_out_of_run_transformation(self):
        from obo.graphs.transformation import run_transformation
        ...
        with (
            patch(
                "obo.graphs.transformation.DefaultPrompts",
                return_value=MagicMock(transformation_instructions=None),
            ),
            patch(
                "obo.graphs.transformation.Prompter"
            ) as mock_prompter_cls,
            patch(
                "obo.graphs.transformation.provision_langchain_model",
...
class TestSourceGraphTransformContentPropagatesFailure:
    """obo/graphs/source.py: transform_content() - the other
    ...
    async def test_add_insight_failure_propagates_out_of_transform_content(self):
        from obo.graphs.source import TransformationState, transform_content
        ...
        with (
            patch(
                "obo.graphs.source.transform_graph.ainvoke",
