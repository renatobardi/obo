...
path validation (obo/ai/key_provider.py sets it directly as
...
from obo.ai.connection_tester import _is_vertex_credentials_file_error
...
class TestTestCredentialClosesOracle:
    ...
    async def test_missing_and_invalid_json_produce_identical_generic_message(
        ...
        with patch(
            "obo.domain.credential.Credential.get",
...
class TestIndividualModelClosesOracle:
    ...
    async def test_missing_credentials_file_returns_generic_message(self, tmp_path):
        ...
        with patch(
            "obo.ai.models.ModelManager", return_value=manager_instance
        ):
