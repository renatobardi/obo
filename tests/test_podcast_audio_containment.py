...
`resolve_contained_audio_path()` (obo/podcasts/audio_paths.py) is
...
from obo.config import PODCASTS_FOLDER
from obo.podcasts.audio_paths import resolve_contained_audio_path
from obo.podcasts.models import PodcastEpisode
...
class TestResolveContainedAudioPath:
    def test_relative_path_inside_root_resolves(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "obo.podcasts.audio_paths.PODCASTS_FOLDER", str(tmp_path)
        )
    ...
    def test_dotdot_escape_is_rejected(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        monkeypatch.setattr(
            "obo.podcasts.audio_paths.PODCASTS_FOLDER", str(root)
        )
    ...
    def test_sibling_directory_with_matching_prefix_is_rejected(
        ...
        real_root.mkdir()
        monkeypatch.setattr(
            "obo.podcasts.audio_paths.PODCASTS_FOLDER", str(real_root)
        )
