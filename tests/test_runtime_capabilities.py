...
class TestEngineRuntimeMissing:
    ...
    def test_crawl4ai_reports_its_env_var_when_absent(self):
        with patch(
            "obo.utils.runtime_capabilities.crawl4ai_available",
            return_value=False,
        ):
            assert (
                engine_runtime_missing("crawl4ai") == "OBO_ENABLE_CRAWL4AI"
            )
    ...
    def test_docling_reports_its_env_var_when_absent(self):
        with patch(
            "obo.utils.runtime_capabilities.docling_available",
            return_value=False,
        ):
            assert engine_runtime_missing("docling") == "OBO_ENABLE_DOCLING"
    ...
    def test_remote_crawl4ai_counts_as_available(self):
        ...
        with patch(
            "obo.utils.runtime_capabilities.crawl4ai_local_ready",
            return_value=False,
        ), patch(
            "obo.utils.runtime_capabilities.crawl4ai_remote_configured",
            return_value=True,
        ):
            assert engine_runtime_missing("crawl4ai") is None
