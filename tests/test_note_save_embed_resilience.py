...
(obo/domain/notebook.py) and its one caller that needs the
...
from obo.domain.notebook import Note
...
class TestNoteSaveEmbedResilience:
    ...
    async def test_save_does_not_raise_when_submission_fails(self):
        ...
        with (
            patch(
                "obo.domain.base.ObjectModel.save",
                new=AsyncMock(),
            ),
            patch(
    ...
    async def test_save_returns_command_id_on_success(self):
        note = Note(title="Test", content="some content")
        with (
            patch("obo.domain.base.ObjectModel.save", new=AsyncMock()),
            patch(
                "obo.domain.notebook.submit_command",
