"""
Unit tests for the obo.graphs module.
...
from obo.domain.notebook import Source
from obo.graphs.prompt import PatternChainState, graph
from obo.graphs.tools import get_current_timestamp
from obo.graphs.transformation import (
    TransformationState,
    run_transformation,
)
from obo.graphs.transformation import (
...
class TestTransformationGraph:
    ...
    def test_transformation_state_structure(self):
        ...
        from obo.domain.notebook import Source
        from obo.domain.transformation import Transformation
...
class TestSaveSourceTitlePreservation:
    ...
    @pytest.mark.asyncio
    @patch("obo.graphs.source.Source.get")
    async def test_custom_title_preserved(self, mock_get):
        ...
        from obo.graphs.source import SourceState, save_source
...
class TestContentProcessDeleteSource:
    ...
    @pytest.mark.asyncio
    @patch("obo.graphs.source.extract_content")
    @patch("obo.graphs.source.ModelManager")
    async def test_uploaded_file_deleted_when_flag_set(
        self, mock_model_manager, mock_extract, tmp_path
    ):
        from content_core.common import ExtractionOutput

        from obo.graphs.source import SourceState, content_process
    ...
    @pytest.mark.asyncio
    @patch("obo.graphs.source.ContentSettings")
    # Runtime present: nothing to fall back from.
    @patch("obo.graphs.source.engine_runtime_missing", return_value=None)
    @patch("obo.graphs.source.extract_content")
    @patch("obo.graphs.source.ModelManager")
    ...
    async def test_unavailable_engine_falls_back_to_auto(
        ...
        OBO_ENABLE_CRAWL4AI leaves the setting pointing at a runtime
        ...
        with patch(
            "obo.graphs.source.engine_runtime_missing",
            side_effect=lambda engine: {
                "crawl4ai": "OBO_ENABLE_CRAWL4AI",
                "docling": "OBO_ENABLE_DOCLING",
            }.get(engine),
        ):
            await content_process(cast(SourceState, state))
...
class TestTransformationModelIdForwarding:
    ...
    @pytest.mark.asyncio
    @patch("obo.graphs.source.transform_graph.ainvoke", new_callable=AsyncMock)
    async def test_source_graph_forwards_model_id(self, mock_ainvoke):
        """obo.graphs.source.transform_content forwards model_id."""
        from obo.domain.transformation import Transformation
        from obo.graphs.source import transform_content
