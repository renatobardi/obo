...
Obo uses a flow-driven release process. Work moves from `ready`
...
- **A local `docker-build-local` tag shadows the pushed image.** Both are
  `ghcr.io/renatobardi/obo:<ver>`, so Phase 6 could verify your own local build
...
- **Judge opt-in runtime gating on a clean image, not the dev venv.** A dev
  ...
  (sets `OBO_ENABLE_DOCLING`/`_CRAWL4AI`; first boot is slow) (v1.13.0 lesson).
