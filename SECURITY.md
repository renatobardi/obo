...
Obo is an actively developed project. Security fixes are applied to the
...
1. Go to the [Security tab](https://github.com/renatobardi/obo/security) of
   the repository.
...
- Steps to reproduce (a proof of concept, affected endpoint/component, or sample
  configuration).
- The Obo version and how you are running it (Docker Compose,
...
Obo is **self-hosted**: you run the API, frontend, and SurrealDB
yourself, and you control the AI provider credentials. Please keep in mind:

- The built-in password middleware (`OBO_PASSWORD`) is a basic access
...
- Misconfiguration of your own deployment (e.g. exposing SurrealDB with default
  credentials, or running without `OBO_ENCRYPTION_KEY`) is a
...
Thank you for helping keep Obo and its users safe.
