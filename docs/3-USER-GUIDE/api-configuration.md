...
> **Credential System**: Obo uses encrypted credentials stored in the database. Each credential connects to a provider and allows you to discover, register, and test models.

---

## Overview

Obo manages AI provider access through a **credential-based system**:
...
Add `OBO_ENCRYPTION_KEY` to your docker-compose.yml:
...
3. The models are now available throughout Obo
...
- Keep `OBO_ENCRYPTION_KEY` — it's still required
...
|---------|---------------|---------------------------|
| Password | None - auth is fully disabled until set | Set `OBO_PASSWORD` |
| Encryption Key | None (must be set) | Set `OBO_ENCRYPTION_KEY` to any secret string |
...
| Error on save | Encryption key not set | Set `OBO_ENCRYPTION_KEY` in docker-compose.yml |
