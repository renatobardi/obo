...
Obo follows a three-tier architecture with clear separation of concerns:
...
- **Migrations**: `.surrealql` files in `obo/database/migrations/`, registered in `AsyncMigrationManager` (auto-run on API startup)
...
LangGraph is a state machine library that orchestrates multi-step AI workflows. Obo uses five core workflows:

### 1. **Source Processing Workflow** (`obo/graphs/source.py`)
...
### 2. **Chat Workflow** (`obo/graphs/chat.py`)
...
### 3. **Ask Workflow** (`obo/graphs/ask.py`)
...
### 4. **Transformation Workflow** (`obo/graphs/transformation.py`)
...
### 5. **Prompt Workflow** (`obo/graphs/prompt.py`)
...
Located in `obo/ai/models.py`, ModelManager handles:
...
**Usage**:
```python
from obo.ai.provision import provision_langchain_model
...
**Repository Pattern**:
- Database access layer (`obo/database/repository.py`)
...
- Located in `obo/database/migrations/`
...
1. Create `obo/graphs/workflow_name.py`
...
1. Create model in `obo/domain/model_name.py`
...
5. Create database migration in `obo/database/migrations/` (and register it in `AsyncMigrationManager`)
