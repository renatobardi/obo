...
- **Status**: Accepted
- **Date**: 2026-07
- **Related**: [ADR-002](ADR-002-external-libraries.md) (external libraries), [Content Processing Engines](../../3-USER-GUIDE/content-processing-engines.md)

## Context

The Content Core 2.x upgrade exposed two selectable extraction runtimes that are far heavier than the rest of the stack:
...
The initial 2.x work bundled Crawl4AI into every image and shipped an OCR toggle that depended on Docling — which was never actually installed, making the toggle a silent no-op and image sources unsupported. So the default image was simultaneously **too heavy** (Chromium for everyone) and **missing** a runtime its own UI advertised.

Baking both into the default image would push it into the multi-GB range and contradict Obo's lean, privacy-first, self-hostable posture (ADR-002 keeps this repo focused on the knowledge layer, delegating extraction to Content Core). Most users need neither runtime.
...
- Two booleans gate them: `OBO_ENABLE_DOCLING` and `OBO_ENABLE_CRAWL4AI`. A remote Crawl4AI server (`CRAWL4AI_API_URL`) needs no local install.
...
- **Offline / air-gapped:** first-boot install needs network. Air-gapped operators leave the flags off. A build-arg to pre-bake the extras is possible future work.
