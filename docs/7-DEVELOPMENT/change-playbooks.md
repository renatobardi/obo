...
Step-by-step guides for common types of changes in the Obo codebase. Each playbook lists the files to touch **in order**, what to do at each step, and what to test.
...
|------|---------|------------|
| 1 | `obo/domain/<model>.py` | Add field with type hint and default value. Follow existing patterns in the class. |
| 2 | `obo/database/migrations/N.surrealql` | Create migration. Use next number in sequence. `DEFINE FIELD` for new fields, `UPDATE` for backfilling existing records. Register it in `AsyncMigrationManager` (`async_migrate.py`) — migrations are not auto-discovered. |
...
| 2 | `obo/graphs/<workflow_name>.py` | Define `StateDict` (TypedDict), node functions, build graph with `StateGraph`. Use `provision_langchain_model()` for model selection. Wrap LLM calls with `classify_error()`. |
...
| 2 | **Read the relevant AGENTS.md** (root, `obo/`, or `frontend/`) and the matching page in `docs/7-DEVELOPMENT/`. They document the rules and gotchas. |
...
| Step | File(s) | What to Do |
|------|---------|------------|
| 1 | `obo/database/migrations/N.surrealql` (+ `N_down.surrealql`) | Write SurrealQL. Use next number in sequence. Check existing migrations for patterns. |
| 2 | `obo/database/async_migrate.py` | Register the new files in `AsyncMigrationManager.__init__` — migrations are hard-coded, not auto-discovered. |
...
|-------|----------|-------------|-------|
| Domain models | `obo/domain/` | Pydantic fields | `tests/` |
| Database | `obo/database/repository.py` | SurrealQL | `tests/` |
| Migrations | `obo/database/migrations/*.surrealql` | SurrealQL | Auto-run on startup |
| AI/LLM | `obo/ai/` | Esperanto types | `tests/` |
| Graphs | `obo/graphs/` | TypedDict state | `tests/` |
