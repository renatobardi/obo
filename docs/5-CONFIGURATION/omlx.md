...
oMLX and SurrealDB both default to **port 8000**. Obo’s database uses `8000`, so running oMLX on the default port will collide.
...
oMLX itself must run on the Mac host. If Obo runs in Docker:

| Setup | Base URL |
|-------|----------|
| Both on host | `http://localhost:11435/v1` |
| Obo in Docker, oMLX on host | `http://host.docker.internal:11435/v1` |
