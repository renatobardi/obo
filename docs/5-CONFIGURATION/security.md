...
Protect your Obo deployment with password authentication and production hardening.

---

## API Key Encryption

Obo encrypts API keys stored in the database using Fernet symmetric encryption (AES-128-CBC with HMAC-SHA256).
...
```bash
# .env or docker.env
OBO_ENCRYPTION_KEY=my-secret-passphrase
```
...
|---------|---------|----------------|
| Password | None - auth is fully disabled until `OBO_PASSWORD` is set | Development only |
| Encryption Key | **None** (must be configured) | Required for API key storage |

**The encryption key has no default.** You must set `OBO_ENCRYPTION_KEY` before using the API key configuration feature. Without it, encrypting/decrypting API keys will fail.
...
```yaml
environment:
  - OBO_PASSWORD_FILE=/run/secrets/app_password
  - OBO_ENCRYPTION_KEY_FILE=/run/secrets/encryption_key
```
...
services:
  obo:
    ...
    pull_policy: always
    environment:
      - OBO_ENCRYPTION_KEY=your-secret-encryption-key
...
OBO_PASSWORD=MySecure2024!Research#Tool
OBO_PASSWORD=Notebook$Dev$2024$Strong!

# Generated (recommended)
OBO_PASSWORD=$(openssl rand -base64 24)
```
...
```bash
# DON'T use these
OBO_PASSWORD=password123
OBO_PASSWORD=opennotebook
OBO_PASSWORD=admin
```
...
Obo's password protection provides **basic access control**, not enterprise-grade security:
...
docker exec obo env | grep OBO_PASSWORD

# Check logs
docker logs obo | grep -i auth
...
echo "Password length: $(echo -n $OBO_PASSWORD | wc -c)"
```
