...
Use any server that implements the OpenAI API format with Obo. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Obo can connect to any server using this format.
...
When Obo runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
services:
  vllm:
    ...
    ports:
      ...
      # 8001 because SurrealDB already publishes 8000. Obo reaches
...
```bash
docker exec -it obo curl http://host.docker.internal:1234/v1/models
```
