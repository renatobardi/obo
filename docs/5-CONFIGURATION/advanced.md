...
```env
# Protect your Obo instance
OBO_PASSWORD=your_secure_password
```
...
Obo uses multiple engines for content extraction. Which one runs is chosen in **Settings → Content Processing** (see the user-guide page on [Content Processing Engines](../3-USER-GUIDE/content-processing-engines.md)); the variables below configure them.
...
Renders JavaScript pages in a local Chromium browser — no API key required. Crawl4AI is **optional**: enable it with `OBO_ENABLE_CRAWL4AI=true` and it installs on first startup (the Chromium download is cached on your data volume). To offload rendering to a remote Crawl4AI server instead — no local install needed:
...
```env
OBO_ENCRYPTION_KEY    # Required for storing credentials
```
...
# Create backup
tar -czf "$BACKUP_DIR/obo-$DATE.tar.gz" \
  /path/to/notebook_data \
  /path/to/surreal_data

# Keep only last 7 days
find "$BACKUP_DIR" -name "obo-*.tar.gz" -mtime +7 -delete

echo "Backup complete: obo-$DATE.tar.gz"
```

Add to cron:
```bash
# Daily backup at 2 AM
0 2 * * * /path/to/backup.sh >> /var/log/obo-backup.log 2>&1
```
...
# On source server
docker compose down
tar -czf obo-migration.tar.gz notebook_data/ surreal_data/

# Transfer to new server
scp obo-migration.tar.gz user@newserver:/path/

# On new server
tar -xzf obo-migration.tar.gz
docker compose up -d
```
