...
Deploy Obo behind nginx, Caddy, Traefik, or other reverse proxies with custom domains and HTTPS.

---

## Simplified Setup (v1.1+)

Starting with v1.1, Obo uses Next.js rewrites to simplify configuration. **You only need to proxy to one port** - Next.js handles internal API routing automatically.
...
# See full base setup: https://github.com/renatobardi/obo/blob/main/docker-compose.yml
services:
  obo:
...
> **Note:** This example only shows the obo and nginx services. You also need a `surrealdb` service. See the [full base docker-compose.yml](https://github.com/renatobardi/obo/blob/main/docker-compose.yml) for the complete setup.
...
Accessing Obo from a different machine on your network:

**Step 1: Get your server IP**
```bash
# On the server running Obo:
hostname -I
# or
ifconfig | grep "inet "
...
services:
  frontend:
    image: ghcr.io/renatobardi/obo_frontend:v1-latest
    pull_policy: always
    environment:
      - API_URL=https://notebook.example.com
    ports:
  ...
  api:
    image: ghcr.io/renatobardi/obo_api:v1-latest
    pull_policy: always
    environment:
      - OBO_ENCRYPTION_KEY=${OBO_ENCRYPTION_KEY}
    ports:
...
1. **Check API_URL is set**:
   ```bash
   docker exec obo env | grep API_URL
   ```
...
2. Check container logs: `docker logs obo`
...
**Cause:** Your reverse proxy has a default timeout (often 30s) that's shorter than Obo's operations.
...
**Version Requirement:**
- **Obo v1.3.2+** is required for file uploads >10MB
...
**Note:** Obo's API includes CORS headers in error responses, but this only works for errors that reach the application. Proxy-level errors (like 413 from nginx) need to be configured at the proxy level.
...
3. **Reverse proxy not forwarding `/api/*` correctly**:
   ...
   location /api/ {
       proxy_pass http://obo:5055/api/;  # Note the trailing slash!
   }
   ```
...
This happens when:
- You have set `OBO_PASSWORD` for authentication
...
9. **Monitor logs** regularly: `docker logs obo`
...
If you're running Obo **version 1.0.x or earlier**, you may need to use the legacy two-port configuration where you explicitly route `/api/*` to port 5055.

**Check your version:**
```bash
docker exec obo cat /app/package.json | grep version
```
