...
Comprehensive list of all environment variables available in Obo.
...
| `OBO_FIREBASE_PROJECT_ID` | Only if `OBO_AUTH_MODE=firebase` | None | Firebase project id used by the backend to verify ID tokens via Google's public JWKS endpoint. No service account is required. See [Firebase Authentication guide](firebase-auth.md). |
| `NEXT_PUBLIC_FIREBASE_API_KEY` | Only if `OBO_AUTH_MODE=firebase` | None | Firebase JS SDK project config (baked into the Next.js build). Safe to ship client-side — see [Firebase Authentication guide](firebase-auth.md). |
...
| `NEXT_PUBLIC_FIREBASE_APP_ID` | Only if `OBO_AUTH_MODE=firebase` | None | Firebase JS SDK project config (baked into the Next.js build; optional). |
...
> **Important**: `OBO_ENCRYPTION_KEY` is required for storing AI provider credentials via the Settings UI. Without it, you cannot save credentials. If you change or lose this key, all stored credentials become unreadable.
...
| `SURREAL_NAMESPACE` | Yes | obo | SurrealDB namespace |
| `SURREAL_DATABASE` | Yes | obo | SurrealDB database name |
...
| Variable | Required? | Default | Description |
|----------|-----------|---------|-------------|
| `OBO_WORKER_MAX_TASKS` | No | 5 | Maximum number of background tasks (source processing, embeddings, podcasts) the worker runs concurrently. Passed to the worker as `--max-tasks` at launch. Set to `1` for **sequential processing** on single-GPU or local-LLM setups, where parallel requests overload the model and trigger rate limits. |

> **Read at worker launch, from the process environment.** In Docker this comes from the container environment — set it under `environment:` in `docker-compose.yml` (or your orchestrator). For local `make worker-start` / `dev-init.sh`, export it in your shell (e.g. `export OBO_WORKER_MAX_TASKS=1`) — it is consumed by the shell before the app loads `.env`, so a value placed only in `.env` will not apply to these local launch paths.
...
| Variable | Required? | Default | Description |
|----------|-----------|---------|-------------|
| `OBO_EMBEDDING_BATCH_SIZE` | No | 50 | Number of texts sent per embedding batch. Lower this for CPU-only or stricter OpenAI-compatible embedding providers. |
| `OBO_MIN_CHUNK_SIZE` | No | 5 | Minimum chunk size in tokens. Chunks below this threshold are dropped before embedding to avoid degenerate single-character fragments that some providers (e.g. llama.cpp) return null embeddings for. Set to `0` to disable filtering. |
...
| Variable | Required? | Default | Description |
|----------|-----------|---------|-------------|
| `OBO_ENABLE_DOCLING` | No | `false` | Install Docling on first startup: unlocks the `docling` document engine, the OCR toggle and image sources. Pulls a large ML stack. |
| `OBO_ENABLE_CRAWL4AI` | No | `false` | Install the local Crawl4AI runtime + a Chromium browser on first startup: unlocks the `crawl4ai` URL engine. Not needed if `CRAWL4AI_API_URL` is set. |
...
The `CCORE_FIRECRAWL_*` variables are passed straight through to the content-core library (its settings are prefixed with `CCORE_`); Obo itself doesn't read them. See [Content Processing Engines](../3-USER-GUIDE/content-processing-engines.md) for how these engines are selected in the UI.
...
| `LANGCHAIN_PROJECT` | No | Obo | LangSmith project name |
...
```
OBO_ENCRYPTION_KEY=my-secret-key
...
```
OBO_ENCRYPTION_KEY=your-strong-secret-key
OBO_PASSWORD=your-secure-password
...
```
OBO_ENCRYPTION_KEY=your-secret-key
API_URL=https://mynotebook.example.com
```
...
echo $OBO_ENCRYPTION_KEY

# Check multiple
env | grep -E "OBO|API_URL"
...
- **Case-sensitive:** `OBO_ENCRYPTION_KEY` ≠ `obo_encryption_key`
- **No spaces:** `OBO_ENCRYPTION_KEY=my-key` not `OBO_ENCRYPTION_KEY = my-key`
...
- [ ] Set `OBO_ENCRYPTION_KEY` in docker-compose.yml
