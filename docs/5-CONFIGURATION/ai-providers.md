...
Obo uses a **credential-based system** for managing AI providers:
...
> **Prerequisite**: You must set `OBO_ENCRYPTION_KEY` in your docker-compose.yml before storing credentials. See [API Configuration](../3-USER-GUIDE/api-configuration.md#encryption-setup) for details.
...
- Reranking is not yet available in Obo (tracked separately).
...
4. Base URL defaults to `http://localhost:11435/v1` (use `http://host.docker.internal:11435/v1` if Obo is in Docker)
...
By default, Obo uses the LLM provider's embeddings. Embedding models are discovered and registered through the same credential system — when you discover models from a credential, embedding models are included alongside language models.
...
3. **Set `OBO_ENCRYPTION_KEY`** in your docker-compose.yml (required for storing credentials)
