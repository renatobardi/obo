...
Get Obo running with **100% local AI** using Ollama. No cloud API keys needed, completely private.
...
Create a new folder `obo-local` and add this file:
...
Open terminal in your `obo-local` folder:
...
docker exec obo-local-ollama-1 ollama pull neural-chat

# OR: Even better quality, more VRAM needed
docker exec obo-local-ollama-1 ollama pull llama2
```
...
```bash
# Show available GPUs
docker exec obo-local-ollama-1 ollama ps
...
docker exec obo-local-ollama-1 ollama list
...
4. In Obo, go to **Settings** → **API Keys**
