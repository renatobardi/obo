...
Run Obo with a **separately installed Ollama** (not via Docker). This avoids Docker running the Ollama service while you use your own local Ollama installation.

## Prerequisites

1. **Docker Desktop** installed (for SurrealDB and Obo)
...
Create a new folder `obo-external-ollama` and add these files:
...
## Step 3: Connect Obo to Host Ollama (1 min)

When Obo runs inside Docker, it cannot reach `localhost:11434` on your host directly. Use the special hostname:

| Host OS | Ollama URL in Obo |
|---------|----------------------------|
...
## Step 4: Start Obo (1 min)

Open terminal in your `obo-external-ollama` folder:
...
3. For Windows/macOS, ensure `host.docker.internal` is reachable from inside the container:
   ```bash
   docker exec <obo_container> curl http://host.docker.internal:11434/api/version
   ```
...
- **Add more models**: Run `ollama pull <model>`, then re-discover from Obo
