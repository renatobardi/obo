# Obo Windows Installation Guide (Native, No Docker)

This guide documents how to install and run [Obo](https://github.com/renatobardi/obo) on Windows **natively without Docker or WSL**.
...
1. **Clone and setup:**
   ...
   git clone https://github.com/renatobardi/obo.git
   cd obo
   uv sync
...
3. **Start the four services**, each in its own terminal, from the `obo` folder.

   > Obo does not ship a launcher script — start the services manually as below (or wrap them in your own `.bat`, see [Optional: one-click launcher](#optional-one-click-launcher)).

   ```batch
   REM Optional: point Obo at a separate data folder (see Issue 4 below).
   REM Set this in each terminal before running, or skip to use ./data.
   set DATA_FOLDER=%USERPROFILE%\Projects\obo-data
...
```
YourProjectsFolder\
├── obo\           # Source code (git clone)
│   ├── .venv\               # Python virtual environment (created by uv)
│   ├── frontend\            # Next.js frontend
│   ├── commands\            # Worker command modules
│   └── .env                 # Your configuration
├── obo-data\      # Data storage (SEPARATE from code!)
│   ├── surrealdb\           # Database files
│   ├── uploads\             # Uploaded documents
│   └── sqlite-db\           # LangGraph checkpoints
└── start-obo.bat  # Optional launcher you create yourself (see below)
```
...
Obo does not ship a launcher, but you can save the following as
`start-obo.bat` (anywhere you like) to start all four services with a
double-click. Adjust `ROOT` and `DATA_ROOT` to match your setup.

```batch
@echo off
REM --- adjust these two paths ---
set ROOT=%USERPROFILE%\Projects\obo
set DATA_ROOT=%USERPROFILE%\Projects\obo-data
...
```batch
set DATA_FOLDER=C:\path\to\obo-data
```

## Configuration Files

### Modifying `obo/config.py`
...
*Tested on Windows 11 ARM64 with Obo v1.6.0*
