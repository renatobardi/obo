...
> **Alternative Registry:** All images are available on both Docker Hub (`ghcr.io/renatobardi/obo`) and GitHub Container Registry (`ghcr.io/renatobardi/obo`). Use GHCR if Docker Hub is blocked or you prefer GitHub-native workflows.
...
The official `docker-compose.yml` is in the root of our repository: [View on GitHub](https://github.com/renatobardi/obo/blob/main/docker-compose.yml)
...
- (Optional) To use database credentials other than the default `root:root`, create a `.env` file next to `docker-compose.yml` with `SURREAL_USER=...` and `SURREAL_PASSWORD=...` — both services pick them up automatically ([.env.example](https://github.com/renatobardi/obo/blob/main/.env.example) shows the full format)

---

## Step 2: Start Services (2 min)

Open terminal in the `obo` folder:
...
✅ obo running on :8502 (UI) and :5055 (API)
```
...
You should see the Obo interface!
...
Done! You now have a fully working Obo instance.
...
curl -o docker-compose.yml https://raw.githubusercontent.com/renatobardi/obo/main/examples/docker-compose-ollama.yml
...
docker exec obo-local-ollama-1 ollama pull mistral
```
...
|----------|---------|---------|
| `OBO_ENCRYPTION_KEY` | Encryption key for credentials | `my-secret-key` |
...
| `SURREAL_NAMESPACE` | Database namespace | `obo` |
| `SURREAL_DATABASE` | Database name | `obo` |
| `API_URL` | API external URL | `http://localhost:5055` |
| `OBO_EMBEDDING_BATCH_SIZE` | Override embedding batch size for stricter/local providers (recommended: `8` for CPU-only local setups) | `50` |
...
- **Issues**: [GitHub Issues](https://github.com/renatobardi/obo/issues)
