...
# Image name (GHCR only)
GHCR_IMAGE := ghcr.io/renatobardi/obo
...
docker-build-local:
	...
	docker build \
		...
		-t $(GHCR_IMAGE):local \
		.
	@echo "✅ Built $(GHCR_IMAGE):$(VERSION) and $(GHCR_IMAGE):local"
	@echo "Run with: docker run -p 5055:5055 -p 3000:3000 $(GHCR_IMAGE):local"
...
docker-push: docker-buildx-prepare
	...
	docker buildx build --pull \
		...
		--push \
		.
	@echo "✅ Pushed version $(VERSION) (latest NOT updated)"
	@echo "  📦 GHCR:"
...
stop-all:
	@echo "🛑 Stopping all Obo services..."
