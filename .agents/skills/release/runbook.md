...
```bash
for ref in ghcr.io/renatobardi/obo:<ver> ghcr.io/renatobardi/obo:<ver>-single ghcr.io/renatobardi/obo:<ver>; do
...
docker exec <that-container> /surreal export --conn http://localhost:8000 \
  --user root --pass root --ns obo --db <that-db> /dev/stdout > /tmp/dev-dump.surql
...
for n in <numbers>; do
  STATE=$(gh api "repos/renatobardi/obo/issues/$n" --jq 'if .pull_request then "pr" else .state end')
