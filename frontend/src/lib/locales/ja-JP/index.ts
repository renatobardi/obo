...
export const jaJP = {
  ...
  connectionErrors: {
    ...
    apiDesc: "Obo APIサーバーに到達できませんでした",
  ...
  notebooks: {
    ...
    emptyTitle: "ソースを追加して始めましょう",
    emptyDesc: "チャット、ノート、スタジオはすべてソースをもとに機能します。まず1つ追加してください。",
    updated: "更新日時",
  ...
  settings: {
    sectionProcessing: "処理",
    sectionPodcastProfiles: "ポッドキャストのプロフィール",
    preferences: "環境設定",
    preferencesDesc: "このブラウザのテーマと言語",
    workspace: "ワークスペース",
    workspaceDesc: "このワークスペースにアクセスできるユーザーを管理します",
    localAccountDesc: "ローカルパスワードでサインインしているため、表示するアカウントプロフィールはありません。",
    ...
    jina: "Jina",
    crawl4ai: "Crawl4AI",
    enableDoclingHint: "Docling は任意です。OBO_ENABLE_DOCLING=true で有効化すると、初回起動時にインストールされます（大容量のダウンロード）。",
    enableCrawl4aiHint: "ローカルの Crawl4AI は任意です。OBO_ENABLE_CRAWL4AI=true で有効化すると初回起動時にインストールされます。リモートサーバーを使う場合は CRAWL4AI_API_URL を設定してください。",
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはOboがアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Oboの新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Obo全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {{models}}。これらがないとOboが正しく機能しない可能性があります。",
  ...
  apiKeys: {
    description: "APIキーをデータベースに安全に保存し、AIプロバイダーをOboで利用可能にします。",
    encryptionRequired: "暗号化キーが設定されていません",
    encryptionRequiredDescription: "OBO_ENCRYPTION_KEY 環境変数に任意の秘密文字列を設定して、データベースへのAPIキーの保存を有効にしてください。",
    configured: "設定済み",
    notConfigured: "未設定",
  ...
  setupBanner: {
    encryptionRequired: "暗号化キーが設定されていません",
    encryptionRequiredDescription: "OBO_ENCRYPTION_KEY 環境変数を設定して、安全な認証情報の保存を有効にしてください。",
