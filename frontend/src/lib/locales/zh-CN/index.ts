...
export const zhCN = {
  ...
  notebooks: {
    ...
    emptyTitle: "添加来源以开始",
    emptyDesc: "聊天、笔记和 Studio 都基于你的来源工作。先添加一个来源。",
    updated: "已更新",
  ...
  settings: {
    sectionProcessing: "处理",
    sectionPodcastProfiles: "播客配置",
    preferences: "偏好设置",
    preferencesDesc: "此浏览器的主题和语言",
    workspace: "工作区",
    workspaceDesc: "管理谁可以访问此工作区",
    localAccountDesc: "您使用本地密码登录，因此没有可显示的账户资料。",
    ...
    jina: "Jina",
    crawl4ai: "Crawl4AI",
    enableDoclingHint: "Docling 为可选项。使用 OBO_ENABLE_DOCLING=true 启用，首次启动时安装（下载较大）。",
    enableCrawl4aiHint: "本地 Crawl4AI 为可选项。使用 OBO_ENABLE_CRAWL4AI=true 启用（首次启动时安装），或设置 CRAWL4AI_API_URL 以使用远程服务器。",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "配置用于 Obo 不同用途的默认模型",
    missingRequiredModels: "缺少必需的模型：{{models}}。如果没有这些模型，Obo 可能无法正常运行。",
  ...
  apiKeys: {
    description: "将 API 密钥安全地存储在数据库中，以在 Obo 中启用 AI 服务商。",
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请将 OBO_ENCRYPTION_KEY 环境变量设置为任意密钥字符串，以启用将 API 密钥存储到数据库。",
    configured: "已配置",
    notConfigured: "未配置",
  ...
  setupBanner: {
    encryptionRequired: "未配置加密密钥",
    encryptionRequiredDescription: "请设置 OBO_ENCRYPTION_KEY 环境变量以启用安全凭据存储。",
