...
export const zhTW = {
  ...
  notebooks: {
    ...
    emptyTitle: "新增來源以開始",
    emptyDesc: "聊天、筆記和 Studio 都以你的來源為基礎運作。先新增一個來源。",
    updated: "已更新",
  ...
  settings: {
    sectionProcessing: "處理",
    sectionPodcastProfiles: "播客設定檔",
    preferences: "偏好設定",
    preferencesDesc: "此瀏覽器的主題和語言",
    workspace: "工作區",
    workspaceDesc: "管理誰可以存取此工作區",
    localAccountDesc: "您使用本機密碼登入，因此沒有可顯示的帳戶設定檔。",
    ...
    jina: "Jina",
    crawl4ai: "Crawl4AI",
    enableDoclingHint: "Docling 為選用項目。使用 OBO_ENABLE_DOCLING=true 啟用，首次啟動時安裝（下載較大）。",
    enableCrawl4aiHint: "本機 Crawl4AI 為選用項目。使用 OBO_ENABLE_CRAWL4AI=true 啟用（首次啟動時安裝），或設定 CRAWL4AI_API_URL 以使用遠端伺服器。",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Obo 的新版本可用。",
    upToDate: "已是最新",
    unknown: "未知",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "設定用於 Obo 不同用途的預設模型",
    missingRequiredModels: "缺少必需的模型：{{models}}。如果没有這些模型，Obo 可能無法正常運行。",
  ...
  apiKeys: {
    description: "將 API 金鑰安全地儲存在資料庫中，以在 Obo 中啟用 AI 服務商。",
    encryptionRequired: "未設定加密金鑰",
    encryptionRequiredDescription: "請將 OBO_ENCRYPTION_KEY 環境變數設定為任意密鑰字串，以啟用將 API 金鑰儲存至資料庫。",
    configured: "已設定",
    notConfigured: "未設定",
  ...
  setupBanner: {
    encryptionRequired: "未設定加密金鑰",
    encryptionRequiredDescription: "請設定 OBO_ENCRYPTION_KEY 環境變數以啟用安全憑據儲存。",
