...
from obo.ai.models import Model, ModelManager
from obo.domain.content_settings import ContentSettings
from obo.domain.notebook import Asset, Source
from obo.domain.transformation import Transformation
from obo.graphs.transformation import graph as transform_graph
from obo.utils.runtime_capabilities import engine_runtime_missing

# Preferred languages for YouTube transcript selection. content-core's own
# default is only ["en", "es", "pt"]; we keep the broader list Obo has
...
def _usable_engine(engine: str, kind: str) -> str:
    ...
    drops OBO_ENABLE_CRAWL4AI/_DOCLING (or a failed on-demand install)
