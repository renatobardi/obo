...
from obo.utils.proxy import ensure_internal_no_proxy
...
from obo.database.async_migrate import AsyncMigrationManager
...
# Parsed once at module load; OBO_MAX_UPLOAD_SIZE_MB changes require a restart.
...
async def lifespan(app: FastAPI):
    ...
    if not get_secret_from_env("OBO_ENCRYPTION_KEY"):
        logger.warning(
            "OBO_ENCRYPTION_KEY not set. "
            "API key encryption will fail until this is configured. "
            "Set OBO_ENCRYPTION_KEY to any secret string."
        )
...
app = FastAPI(
    title="Obo API",
    description="API for Obo - Research Assistant",
    lifespan=lifespan,
)
...
logger.info(
    ...
    "(set OBO_MAX_UPLOAD_SIZE_MB to change)"
)
...
@app.exception_handler(OboError)
async def obo_error_handler(request: Request, exc: OboError):
    return JSONResponse(
        status_code=500,
        content={"detail": str(exc)},
        headers=_cors_headers(request),
...
@app.get("/")
async def root():
    return {"message": "Obo API is running"}
