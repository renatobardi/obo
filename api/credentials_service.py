...
from obo.ai.connection_tester import normalize_anthropic_compatible_base_url
from obo.ai.model_discovery import (
    ANTHROPIC_FALLBACK_MODELS,
    OPENROUTER_AUDIO_MODELS,
    classify_model_type,
    fetch_anthropic_model_ids,
)
from obo.ai.provider_registry import PROVIDERS
from obo.domain.credential import Credential
from obo.utils.encryption import get_secret_from_env
from obo.utils.url_validation import (
...
# registry (obo/ai/provider_registry.py — the source of truth).
...
def require_encryption_key() -> None:
    ...
    if not get_secret_from_env("OBO_ENCRYPTION_KEY"):
        raise ValueError(
            "Encryption key not configured. "
            "Set OBO_ENCRYPTION_KEY to enable storing API keys."
        )
...
async def get_provider_status() -> dict:
    ...
    """
    encryption_configured = bool(get_secret_from_env("OBO_ENCRYPTION_KEY"))
...
async def test_credential(credential_id: str) -> dict:
    ...
    try:
        ...
        from obo.ai.connection_tester import TEST_MODELS
...
async def register_models(credential_id: str, models_data: list) -> dict:
    ...
    from obo.ai.models import Model
...
async def migrate_from_provider_config() -> dict:
    ...
    from obo.domain.provider_config import ProviderConfig
